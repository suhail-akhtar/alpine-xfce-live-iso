...
create_image_iso() {
	...
	if [ "$ARCH" = ppc64le ]; then
		...
		xorrisofs \
			...
			-rational-rock \
			-sysid LINUX \
			-volid "prolight-${profile_abbrev:-$PROFILE} $RELEASE $ARCH" \
			$_isolinux \
			$_efiboot \
			-follow-links \
			${iso_opts} \
...
profile_base() {
	...
	apks="alpine-base busybox chrony kmod dhcpcd doas e2fsprogs
